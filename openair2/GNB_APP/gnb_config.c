...
f1ap_gnb_du_system_info_t *get_sys_info(NR_BCCH_BCH_Message_t *mib, const NR_BCCH_DL_SCH_Message_t *sib1, seq_arr_t *du_SIBs)
{
  ...
  if (du_SIBs) {
    for (int i = 0; i < du_SIBs->size; i++) {
      ...
      // DU SIBs not included in this message are SIB19 and the PWS SIBs (SIB6/7/8)
      AssertFatal(si->SIB_type == NR_SIB_19 || si->SIB_type == NR_SIB_6 || si->SIB_type == NR_SIB_7
                      || si->SIB_type == NR_SIB_8,
...
static seq_arr_t *fill_du_sibs(paramdef_t *GNBparamarray)
{
  ...
  for (int i = 0; i < GNBparamarray[GNB_DU_SIBS_IDX].numelt; i++) {
    ...
    // The PWS SIBs (SIB6/7/8) carry no RRC container from the CU: their content is built
    // locally from the "pws" configuration section, so they are DU SIBs here.
    const bool is_pws_sib = sib_value == NR_SIB_6 || sib_value == NR_SIB_7 || sib_value == NR_SIB_8;
    AssertFatal(is_pws_sib
                    || ((sib_value > NR_SIB_9 && sib_value < NR_SIB_21) && sib_value != NR_SIB_11 && sib_value != NR_SIB_16),
